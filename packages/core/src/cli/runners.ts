...
export async function runListPatches(opts: Pick<CommandOptions, "cwd" | "patchesDir" | "json">): Promise<void> {
  if (!opts.targetsFile) {
    throw new Error("portfolio mode requires --targets-file.");
  }

  const targets = JSON.parse(readFileSync(opts.targetsFile, "utf8")) as Array<{
    cwd: string;
    label?: string;
    cveId?: string;
    inputPath?: string;
  ...
  const report = await remediatePortfolio({
    ...
    changeRequest: buildChangeRequestOptions(opts),
  });
  ...
  process.stdout.write(`Targets: ${report.targets.length}\n`);
  process.stdout.write(`Successful targets: ${report.successCount}\n`);
  process.stdout.write(`Failed targets: ${report.failedCount}\n`);
  if (report.changeRequests?.length) {
    process.stdout.write(`Change requests: ${report.changeRequests.length}\n`);
  }
}
