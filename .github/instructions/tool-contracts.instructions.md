...
- `check-inventory`, `apply-version-bump`, `apply-package-override`, and `apply-patch-file` must accept `packageManager` and auto-detect from lockfile when omitted.
...
- `apply-package-override` must write package-manager-native override fields: npm `overrides`, pnpm `pnpm.overrides`, yarn `resolutions`.
